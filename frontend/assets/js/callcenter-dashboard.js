...
// Update user info in header
// Update user info in header
// Update user info handled by Layout.
async function initCallCenterDashboard() {
    // Layout.render handles user info now, but we can keep this or rely on Layout.
    // Layout.render calls updateHeaderUserInfo() which does this.
    // So we just load data.
    initInquiriesChart();

    // We can call these parallelly
    await Promise.all([
        loadDashboardStats(),
        loadRecentInquiries()
    ]);
