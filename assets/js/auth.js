...
class Auth {
    ...
    login(email, password, remember = false) {
        const userKey = Object.keys(DEMO_USERS).find(key => DEMO_USERS[key].email === email);
        if (!userKey || DEMO_USERS[userKey].password !== password) {
            return { success: false, message: 'Invalid email or password' };
        }

        const user = DEMO_USERS[userKey];
        const session = { user, timestamp: new Date().toISOString(), remember };
        ...
        const redirectUrl = getNavigationPath('select-portal.html');
    ...
    loadSession() {
        ...
        if (sessionData) {
            try {
                this.currentUser = JSON.parse(sessionData).user;
            } catch {
                this.clearSession();
            }
        }
